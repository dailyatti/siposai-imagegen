...
export const ImageCard: React.FC<ImageCardProps> = ({ item, onUpdateConfig, onProcess, onRemove, onEdit, onMultiVariant, isSelected, onToggleSelection, onQuickRemoveText }) => {
    ...
    const downloadImage = async (url: string, format: string, filename: string) => {
        try {
    ...
    const handleDownload = () => {
        // Determine source URL: Processed > Preview (Original)
        const sourceUrl = item.processedUrl || item.previewUrl;
        if (!sourceUrl) return;

        const ext = item.targetFormat.split('/')[1]; // jpg, png, webp
        const filename = item.customOutputName && item.customOutputName.trim() !== ''
            ? item.customOutputName
            : `banana_${item.status === ProcessingStatus.SUCCESS ? 'remaster' : 'original'}_${item.originalMeta.name.split('.')[0]}`;

        // Trigger download with conversion
        downloadImage(sourceUrl, ext, filename);
    };
                ...
                <div className="mt-6 pt-4 border-t border-slate-800 flex items-end justify-between relative z-0">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-widest shadow-lg transition-all transform active:scale-95 ${isSuccess ? 'bg-emerald-500 hover:bg-emerald-400 text-white shadow-emerald-500/20' : 'bg-slate-700 hover:bg-slate-600 text-slate-200'}`}
                        >
                            <Download className="w-3.5 h-3.5" />
                            <span>{isSuccess ? t('export') : 'Save'}</span>
                        </button>
                    </div>
                </div>
